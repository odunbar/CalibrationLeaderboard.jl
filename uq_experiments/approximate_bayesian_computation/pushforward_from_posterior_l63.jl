# Approximate Bayesian Computation — pushforward stage, L63
#
...
# forward n_pushforward_samples resampled from it through the Lorenz-63
# forward map (fresh IC perturbation per sample). The raw pool size is often
# small and grows unevenly (ABC rejection sampling), which makes coverage
...
function pushforward_one(cfg, N_ens, rng_idx, output_dir)
    ...
    phi_stored, pool_sizes, prior, x0, ic_cov_sqrt, lorenz_config_settings, observation_config = JLD2.jldopen(fn, "r") do f
    ...
    for (ki, k) in enumerate(k_values)
        ...
        @info "Pushforward k=$(k), N_ens=$(N_ens), rng_idx=$(rng_idx): $(n_pushforward_samples) Lorenz63 evals (Gaussian-resampled, pool size $(pool_sizes[k]))"
