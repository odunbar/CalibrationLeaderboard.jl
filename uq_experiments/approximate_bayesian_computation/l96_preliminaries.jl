# Approximate Bayesian Computation — L96 preliminaries (truth data + observations)
#
# Computes the perfect-model truth trajectory and target summary statistics
# once, serially, for one force_case (const / vec / flux), and saves them to
# output/l96_computed_preliminaries_<force_case>.jld2. Run this before the
# calibrate stage — extracting it out of calibrate_l96.jl avoids every array
# task racing to compute and write the same file.
#
# `y` (the observation/summary-statistic vector) and `R` (its estimated
# covariance from independent windows) computed here play exactly the role of
# the reference notebooks' `moments_true_mean` and `moments_true_Rd` — ABC's
# rejection criterion compares a candidate's simulated summary statistic
# against `y` using `R`.
#
...
# Mirrors calibrate_l96.jl's force_case_setup (per-case, deterministic) —
# only nx/phi/T/inff are needed here to build the truth trajectory.
