# Approximate Bayesian Computation — leaderboard netcdf writer
#
# Loads the stored accepted-sample-pool history and pushforward output samples
# from every calibrate cell, computes per-round pool mean/covariance
# (parameter space, from the raw accepted pool) and output-space coverage +
# budget-to-target (from the Gaussian-resampled pushforward samples written by
...
# calibrate_emulate_sample/hpc-variant/minimal_leaderboard_nc.jl and
# uq_experiments/GaussNewtonKalmanInversion/exp_to_leaderboard.jl.
#
...
function main()
    ...
    # array task killed mid-JLD2.save by a time/mem limit) — isfile(fn) alone
    # can't detect that, and jldopen throws InvalidDataException ("Did not
    # find a Superblock") on such a file. Treat that as no pushforward for
    # this cell rather than aborting the whole leaderboard run. A cell can
    # also legitimately have no pushforward at all if its accepted-sample
    # pool never reached the minimum size needed for a Gaussian fit within
    # cfg.max_iter rounds (see pushforward_from_posterior_l*.jl) — same
    # handling applies.
    ...
    for (N_ens, rng_idx) in valid_items
        ...
        n_k      = max(n_k, maximum(pf_k_values))
    ...
    k_description               = "Number of ABC rounds completed (1-indexed); rounds before the pool reached a usable size are omitted (NaN)"
    ...
    pool_size_description       = "Number of accepted ABC samples in the posterior pool at round k_iter (NaN where pool was below the minimum size for a Gaussian pushforward fit)"
    ...
    post_mean_v.attrib["description"] = "mean(accepted pool) in parameter (constrained) space at each ABC round"
    ...
    post_cov_v.attrib["description"] = "cov(accepted pool) in parameter (constrained) space at each ABC round"
    post_cov_v[:, :, :, :, :] = post_cov_arr

    pool_size_v = defVar(ds, "pool_size", Float64, ("random_seed", "ensemble_size", "k_iter"); fillvalue = NaN)
    pool_size_v.attrib["description"] = pool_size_description
    pool_size_v[:, :, :] = pool_size_arr
