...
# resampled from it through the Lorenz-96 forward map (fresh IC perturbation
# per sample). N_ens is often small relative to the forcing parameterization
# (e.g. flux-force has 61 params), which makes coverage estimated directly
# from it noisy; resampling from the Gaussian implied by the ensemble's own
# mean/cov — the quantity GNKI actually reports as its posterior
# approximation — gives a much larger, fixed-size sample set (matching
...
function pushforward_one(cfg, N_ens, rng_idx, output_dir)
    ...
    ϕ_stored, prior, x0, ic_cov_sqrt, lorenz_config_settings, observation_config, truth_phi, phi_structure, sample_range =
    ...
    for k in 1:K
        ...
        @info "Pushforward k=$(k), N_ens=$(N_ens), rng_idx=$(rng_idx): $(n_pushforward_samples) Lorenz96 evals (Gaussian-resampled)"
