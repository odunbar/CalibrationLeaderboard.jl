#!/bin/bash
# Submit preliminaries + calibrate + pushforward + leaderboard for the L96 flux-force case.
#
...
CALIB_JID=$(sbatch --parsable \
		   ...
		   --export=ALL,SCRIPT=calibrate_l96.jl,EXPERIMENT=l96_flux,CALIBRATE_DATE=${RUN_DATE} \
		   calibrate_array.sbatch)
echo "  calibrate job ID: ${CALIB_JID}"

echo "=== Submitting pushforward_from_posterior (L96 flux-force, after ${CALIB_JID}) ==="
PUSHFWD_JID=$(sbatch --parsable \
		 -A esm \
		 --job-name="pushfwd_${LABEL}" \
		 --dependency=afterany:${CALIB_JID} \
		 --export=ALL,EXPERIMENT=l96_flux,CALIBRATE_DATE=${RUN_DATE} \
...
# sbatch --parsable -A esm --job-name="pushfwd_l96f" --export=ALL,EXPERIMENT=l96_flux,CALIBRATE_DATE=<yyyy-mm-dd> pushforward_from_posterior.sbatch

# sbatch -A esm --job-name="leaderboard_l96f" --export=ALL,EXPERIMENT=l96_flux,CALIBRATE_DATE=<yyyy-mm-dd> exp_to_leaderboard.sbatch
