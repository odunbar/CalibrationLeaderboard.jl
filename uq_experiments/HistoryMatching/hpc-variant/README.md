...
points at the method directory's `Project.toml`. (See the
`slurm-pipeline-handler` skill — this mirrors `uq_experiments/GaussNewtonKalmanInversion/`.)

There is no separate emulate_sample stage — the emulation (GP fit) happens
inside every wave of `calibrate_<MODEL>.jl`, and the NROY draw at wave `k` is
itself the sample set that gets pushed forward and scored (see the top-level
README's "Algorithm summary").
...
History Matching wave loop (LHS wave 1, rejection-sampled NROY waves
thereafter) to `max_waves` for every `(N_ens, rng_idx)` cell, storing each
wave's NROY sample set. `pushforward_from_posterior` pushes those stored
samples through the Lorenz forward map and saves output samples back into
...
directory local runs use) — `../output/slurm/` for logs,
`../output/history-matching_<date>/` for calibrate results, matching the
top-level README's paths exactly.
