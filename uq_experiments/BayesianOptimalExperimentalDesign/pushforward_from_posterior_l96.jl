# BayesianOptimalExperimentalDesign — pushforward stage, L96 (const / vec / flux forcing)
#
# For each calibrate cell and each completed GBOED iteration k: push the
# ST-MCMC posterior sample set drawn by calibrate_l96.jl's loop through the
# Lorenz-96 forward map (fresh IC perturbation per sample). Also records
# forcing-space samples (matching
...
#
# See pushforward_from_posterior_l63.jl's header for why there is no
# "fewer samples than expected" early-stop/NaN-pad handling to replicate
# from HistoryMatching (GBOED's ST-MCMC never comes up short).
#
...
function pushforward_one(cfg, N_ens, rng_idx, output_dir)
    ...
    for (ki, k) in enumerate(k_values)
        theta_samples = posteriors_by_k[k]   # nu x n_samples_k
