# BayesianOptimalExperimentalDesign — leaderboard netcdf writer
#
# Loads each cell's ST-MCMC posterior sample sets (drawn by
# calibrate_<MODEL>.jl's GBOED loop) and pushforward output samples (written
# by pushforward_from_posterior_<MODEL>.jl), computes per-iteration
# parameter-space mean/cov and R-whitened PCA output coverage +
# budget-to-target, and writes the leaderboard netcdf (full + minimal).
# k_iter here indexes GBOED ITERATIONS, not EKI/RF training iterations —
# output_budget_to_target = N_ens * k_iter (iteration 1 = initial LHS design,
# no acquisition yet; see experiment_config.jl's header for why this keeps
# the budget convention identical to every other UQ method). Run serially
# after all pushforward cells have completed.
#
# Output-space coverage is R-whitened PCA coverage (same metric definition as
# uq_experiments/HistoryMatching/exp_to_leaderboard.jl,
...
function main()
    ...
    for (N_ens, rng_idx) in valid_items
        ...
        for (ki, k) in enumerate(pf_k_values)
            theta_k = posteriors_by_k[k]   # n_params x n_samples (ST-MCMC posterior draw at GBOED iteration k)
            # ST-MCMC always returns exactly cfg.n_posterior_samples, so this
            # guard is not expected to trigger in practice — kept only as a
            # cheap safety net against an underdetermined mean/cov or an
            # empty quantile in a misconfigured/degenerate run.
    ...
    # ── Budget-to-target (smallest N_ens*k_iter reaching calibrated coverage) ─
    ...
    post_mean_v.attrib["description"] = "mean(ST-MCMC posterior sample set) in parameter (constrained) space at each GBOED iteration"
    ...
    post_cov_v.attrib["description"] = "cov(ST-MCMC posterior sample set) in parameter (constrained) space at each GBOED iteration"
