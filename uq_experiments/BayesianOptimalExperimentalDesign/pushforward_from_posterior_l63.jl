# BayesianOptimalExperimentalDesign — pushforward stage, L63
#
# For each calibrate cell and each completed GBOED iteration k: push the
# ST-MCMC posterior sample set drawn by calibrate_l63.jl's loop through the
# Lorenz-63 forward map (fresh IC perturbation per sample). Appends output
# samples into the same results JLD2 calibrate_l63.jl wrote.
#
# Simpler than HistoryMatching's pushforward stage: GBOED's ST-MCMC always
# returns exactly cfg.n_posterior_samples samples (no rejection-sampling
# shortfall failure mode), so there is no "fewer samples than expected"
# early-stop/NaN-pad handling to replicate — the min()/NaN-pad below is kept
# only as a cheap safety net if n_posterior_samples and n_pushforward_samples
# are ever configured to differ.
#
...
# GaussianProcesses.GPE, from calibrate_l63.jl). This script never touches
...
function pushforward_one(cfg, N_ens, rng_idx, output_dir)
    ...
    # Load only the keys actually needed here (avoids deserializing
    # `gps_by_k`, which can be large and isn't used by this stage).
    ...
    for (ki, k) in enumerate(k_values)
        theta_samples = posteriors_by_k[k]   # 2 x n_samples_k
