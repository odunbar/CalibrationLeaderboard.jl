# BayesianOptimalExperimentalDesign HPC variant
...
in the `slurm-pipeline-handler` skill.)

There is no separate emulate_sample stage — like `HistoryMatching` and
`GaussNewtonKalmanInversion`, the GP fit, ST-MCMC posterior sampling, and (here
additionally) the EIG acquisition-batch optimization all live inside every
iteration of `calibrate_l*.jl` (see the top-level README).
...
### L63
```
l63_preliminaries  ─(afterok)→  calibrate_array (GBOED loop)  ─(afterany)→  pushforward_from_posterior  ─(afterany)→  exp_to_leaderboard
```

### L96 (const / vec / flux)
```
l96_preliminaries  ─(afterok)→  calibrate_array (GBOED loop)  ─(afterany)→  pushforward_from_posterior  ─(afterany)→  exp_to_leaderboard
```
...
racing to compute and write the same file. `calibrate_array` then runs the
GBOED loop (initial LHS design + GP fit + ST-MCMC posterior, then
`max_iters - 1` EIG-optimized acquisition rounds) for every `(N_ens, rng_idx)`
cell, storing the GP and ST-MCMC posterior sample set at each iteration.
`pushforward_from_posterior` pushes the stored posterior sample sets through
the Lorenz forward map and saves output samples back into each cell's results
JLD2. `exp_to_leaderboard` loads all cells serially and writes the leaderboard
NetCDF.
...
| `calibrate_array.sbatch` | array (1–180) | One task per `(N_ens, rng_idx)` cell; runs the GBOED loop; model-agnostic (SCRIPT env var) |
...
directory local runs use) — `../output/slurm/` for logs, `../output/boed_<date>/`
...
same for all four cases — l63/l96_const use `N_ens_sizes = 4:2:20`, l96_vec/
l96_flux use `N_ens_sizes = 50:5:90`, both 9 values). If you change
`N_ens_sizes` or `n_repeats` in `experiment_config.jl` (the single copy in the
parent directory), update the upper bound to `length(N_ens_sizes) * n_repeats`
in both `calibrate_array.sbatch` and `pushforward_from_posterior.sbatch`. The
`%100` suffix caps concurrent tasks as a cluster-courtesy limit; raise or
remove it if you want faster turnaround.
