...
function write_results_nc(
    ...
    v_ens.attrib["description"] = "Number of ensemble members (restarts for gradient-based methods)"
    v_rmse        = defVar(ds, "rmse_target",    Float64, ("rmse_target",))
    v_rmse.attrib["description"] = "Target accuracy level (root mean square error)"
    v_alg         = defVar(ds, "algorithm_type", String,  ("algorithm_type",))
    v_metric      = defVar(ds, "metric", Float64,
        ("random_seed", "ensemble_size", "rmse_target", "algorithm_type"),
        fillvalue = NaN)
    v_metric.attrib["description"] = "Number of forward model evaluations (algorithm cost)"
