...
EXP_ID=${1:-}
LABEL="l63_${SCORE_KIND:-dsm}_${BUDGET_MODE:-fair}${EXP_ID:+_${EXP_ID}}"
RUN_DATE=$(date +%Y-%m-%d)
...
#
# preliminaries:
#   sbatch -A esm --export=ALL,SCRIPT=l63_preliminaries.jl,EXPERIMENT=l63,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=fair preliminaries.sbatch
#
# one array cell (smoke test):
#   sbatch -A esm --array=1-1 --export=ALL,SCRIPT=run_l63_sblm.jl,EXPERIMENT=l63,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=fair run_array.sbatch
#
# full array:
#   sbatch -A esm --array=1-900 --export=ALL,SCRIPT=run_l63_sblm.jl,EXPERIMENT=l63,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=fair run_array.sbatch
#
# leaderboard only:
#   sbatch -A esm --export=ALL,EXPERIMENT=l63,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=fair leaderboard.sbatch
#
# the other arms:
#   SCORE_KIND=dsm       BUDGET_MODE=ensemble  bash submit_l63.sh
#   SCORE_KIND=gaussian BUDGET_MODE=fair      bash submit_l63.sh
# ---------------------------------------------------------------------------
