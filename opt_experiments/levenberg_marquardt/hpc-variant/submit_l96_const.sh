#!/bin/bash
# Submit the Levenberg-Marquardt L96 const-force pipeline.
# Dependency chain: preliminaries →(afterok)→ run_array →(afterok)→ leaderboard
#
# N_TASKS = length(rmse_targets) * n_repeats = 3 * 100 = 300
# If rmse_targets or n_repeats change in experiment_config.jl, update --array below.
#
# Usage: bash submit_l96_const.sh [EXP_ID]
#   EXP_ID (optional): label appended to SLURM job names,
#                      e.g. "run2" → jobs appear as "run_l96c_run2".

set -euo pipefail

EXP_ID=${1:-}
LABEL="l96c${EXP_ID:+_${EXP_ID}}"
RUN_DATE=$(date +%Y-%m-%d)
DIR="$(cd "$(dirname "$0")" && pwd)"

cd "$DIR"
mkdir -p ../output/slurm

echo "NOTE: This script does not precompile. Run bash submit_precompile.sh first"
echo "      if you haven't done so recently (e.g. after a fresh checkout or package update)."
echo "  run_date pinned to ${RUN_DATE} for all jobs in this pipeline."

echo "=== Submitting preliminaries (L96 const-force) ==="
PRELIM_JID=$(sbatch --parsable \
                    -A esm \
                    --job-name="prelim_${LABEL}" \
                    --export=ALL,SCRIPT=l96_preliminaries.jl,EXPERIMENT=l96_const,RUN_DATE=${RUN_DATE} \
                    preliminaries.sbatch)
echo "  preliminaries job ID: ${PRELIM_JID}"

echo "=== Submitting run_array (L96 const-force, lm, after ${PRELIM_JID}) ==="
RUN_JID=$(sbatch --parsable \
                 -A esm \
                 --job-name="run_${LABEL}" \
                 --array=1-300 \
                 --dependency=afterok:${PRELIM_JID} \
                 --kill-on-invalid-dep=yes \
                 --export=ALL,SCRIPT=run_l96_lm.jl,EXPERIMENT=l96_const,RUN_DATE=${RUN_DATE} \
                 run_array.sbatch)
echo "  run_array job ID: ${RUN_JID}"

echo "=== Submitting leaderboard (L96 const-force, after ${RUN_JID}) ==="
LB_JID=$(sbatch --parsable \
                -A esm \
                --job-name="leaderboard_${LABEL}" \
                --dependency=afterany:${RUN_JID} \
                --kill-on-invalid-dep=yes \
                --export=ALL,EXPERIMENT=l96_const,RUN_DATE=${RUN_DATE} \
                leaderboard.sbatch)
echo "  leaderboard job ID: ${LB_JID}"

echo "=== Done. Monitor with: squeue -u \$USER ==="
