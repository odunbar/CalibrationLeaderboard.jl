#!/bin/bash
# Submit the adam L96 flux-force pipeline.
...
PRELIM_JID=$(sbatch --parsable \
                    ...
                    --export=ALL,SCRIPT=l96_preliminaries.jl,EXPERIMENT=l96_flux,RUN_DATE=${RUN_DATE} \
                    preliminaries.sbatch)
echo "  preliminaries job ID: ${PRELIM_JID}"

echo "=== Submitting run_array (L96 flux-force, adam, after ${PRELIM_JID}) ==="
RUN_JID=$(sbatch --parsable \
                 -A esm \
                 --job-name="run_${LABEL}" \
                 --dependency=afterok:${PRELIM_JID} \
                 --kill-on-invalid-dep=yes \
                 --export=ALL,SCRIPT=run_l96_adam.jl,EXPERIMENT=l96_flux,RUN_DATE=${RUN_DATE} \
                 run_array.sbatch)
...
LB_JID=$(sbatch --parsable \
                ...
                --kill-on-invalid-dep=yes \
                --export=ALL,EXPERIMENT=l96_flux,RUN_DATE=${RUN_DATE} \
                leaderboard.sbatch)
