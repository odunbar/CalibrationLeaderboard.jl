# adam — L96 opt experiment
# Adam on (y - G(θ))' R⁻¹ (y - G(θ)) with ForwardDiff.jl Jacobians.
...
# N_ens IC perturbations averaged each step; N_ens=1 is Adam with a single IC sample.
# Cost: outer_iter × N_ens × (nu + 1) forward-model evaluations.
#
# Local: EXPERIMENT=l96_const julia --project=. run_l96_adam.jl
#        EXPERIMENT=l96_vec   julia --project=. run_l96_adam.jl
#        EXPERIMENT=l96_flux  julia --project=. run_l96_adam.jl
# One cell: EXPERIMENT=l96_const julia --project=. run_l96_adam.jl <task_idx>
...
# Single Adam trajectory; N_ens IC perturbations averaged each step to
# reduce gradient noise.  N_ens=1 is Adam with a single IC sample.
# Cost: outer_iter × N_ens × (nu + 1) forward-model evaluations.
