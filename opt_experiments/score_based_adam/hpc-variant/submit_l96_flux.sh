#!/bin/bash
# Submit the score-based Adam L96 flux-force pipeline.
...
#
# Note: flux-force trains a Flux.jl NN per cell and falls back to ForwardDiff
# through it on collapsed-attractor iterations (nu ≈ 61 params, nx=100). If
# tasks time out, resubmit with --time=08:00:00 (or longer).
...
echo "=== Submitting run_array (L96 flux-force, sbadam, after ${PRELIM_JID}) ==="
RUN_JID=$(sbatch --parsable \
                 ...
                 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_flux,RUN_DATE=${RUN_DATE},SCORE_KIND=${SCORE_KIND},BUDGET_MODE=${BUDGET_MODE} \
                 run_array.sbatch)
...
#   sbatch -A esm --array=1-1 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_flux,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial run_array.sbatch
#
# full array:
#   sbatch -A esm --array=1-900 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_flux,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial run_array.sbatch
#
# leaderboard only:
