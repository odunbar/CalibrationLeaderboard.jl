#!/bin/bash
# Submit the score-based Adam L63 pipeline.
# Dependency chain: preliminaries →(afterok)→ run_array →(afterany)→ leaderboard
#
...
EXP_ID=${1:-}
LABEL="l63_${SCORE_KIND:-dsm}_${BUDGET_MODE:-serial}${EXP_ID:+_${EXP_ID}}"
RUN_DATE=$(date +%Y-%m-%d)
...
PRELIM_JID=$(sbatch --parsable \
                    ...
                    --export=ALL,SCRIPT=l63_preliminaries.jl,EXPERIMENT=l63,RUN_DATE=${RUN_DATE},SCORE_KIND=${SCORE_KIND},BUDGET_MODE=${BUDGET_MODE} \
                    preliminaries.sbatch)
echo "  preliminaries job ID: ${PRELIM_JID}"

echo "=== Submitting run_array (L63, sbadam, after ${PRELIM_JID}) ==="
RUN_JID=$(sbatch --parsable \
                 ...
                 --export=ALL,SCRIPT=run_l63_sbadam.jl,EXPERIMENT=l63,RUN_DATE=${RUN_DATE},SCORE_KIND=${SCORE_KIND},BUDGET_MODE=${BUDGET_MODE} \
                 run_array.sbatch)
...
LB_JID=$(sbatch --parsable \
                ...
                --kill-on-invalid-dep=yes \
                --export=ALL,EXPERIMENT=l63,RUN_DATE=${RUN_DATE},SCORE_KIND=${SCORE_KIND},BUDGET_MODE=${BUDGET_MODE} \
                leaderboard.sbatch)
...
#
# preliminaries:
#   sbatch -A esm --export=ALL,SCRIPT=l63_preliminaries.jl,EXPERIMENT=l63,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial preliminaries.sbatch
#
# one array cell (smoke test):
#   sbatch -A esm --array=1-1 --export=ALL,SCRIPT=run_l63_sbadam.jl,EXPERIMENT=l63,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial run_array.sbatch
#
# full array:
#   sbatch -A esm --array=1-900 --export=ALL,SCRIPT=run_l63_sbadam.jl,EXPERIMENT=l63,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial run_array.sbatch
#
# leaderboard only:
#   sbatch -A esm --export=ALL,EXPERIMENT=l63,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial leaderboard.sbatch
#
# the other arms:
#   SCORE_KIND=dsm       BUDGET_MODE=parallel  bash submit_l63.sh
#   SCORE_KIND=gaussian BUDGET_MODE=serial      bash submit_l63.sh
# ---------------------------------------------------------------------------
