#!/bin/bash
# Submit the score-based Adam L96 const-force pipeline.
...
#                      e.g. "run2" → jobs appear as "run_l96c_run2".
...
PRELIM_JID=$(sbatch --parsable \
                    ...
                    --export=ALL,SCRIPT=l96_preliminaries.jl,EXPERIMENT=l96_const,RUN_DATE=${RUN_DATE},SCORE_KIND=${SCORE_KIND},BUDGET_MODE=${BUDGET_MODE} \
                    preliminaries.sbatch)
echo "  preliminaries job ID: ${PRELIM_JID}"

echo "=== Submitting run_array (L96 const-force, sbadam, after ${PRELIM_JID}) ==="
RUN_JID=$(sbatch --parsable \
                 ...
                 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_const,RUN_DATE=${RUN_DATE},SCORE_KIND=${SCORE_KIND},BUDGET_MODE=${BUDGET_MODE} \
                 run_array.sbatch)
echo "  run_array job ID: ${RUN_JID}"

echo "=== Submitting leaderboard (L96 const-force, after ${RUN_JID}) ==="
LB_JID=$(sbatch --parsable \
                -A esm \
                --job-name="leaderboard_${LABEL}" \
                --dependency=afterany:${RUN_JID} \
                --kill-on-invalid-dep=yes \
                --export=ALL,EXPERIMENT=l96_const,RUN_DATE=${RUN_DATE},SCORE_KIND=${SCORE_KIND},BUDGET_MODE=${BUDGET_MODE} \
                leaderboard.sbatch)
...
#   sbatch -A esm --array=1-1 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_const,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial run_array.sbatch
#
# full array:
#   sbatch -A esm --array=1-900 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_const,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial run_array.sbatch
#
# leaderboard only:
