#!/bin/bash
# Submit the score-based Adam L96 vec-force pipeline.
...
EXP_ID=${1:-}
LABEL="l96v_${SCORE_KIND:-dsm}_${BUDGET_MODE:-serial}${EXP_ID:+_${EXP_ID}}"
RUN_DATE=$(date +%Y-%m-%d)
...
echo "=== Submitting run_array (L96 vec-force, sbadam, after ${PRELIM_JID}) ==="
RUN_JID=$(sbatch --parsable \
                 ...
                 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_vec,RUN_DATE=${RUN_DATE},SCORE_KIND=${SCORE_KIND},BUDGET_MODE=${BUDGET_MODE} \
                 run_array.sbatch)
...
#
# preliminaries:
#   sbatch -A esm --export=ALL,SCRIPT=l96_preliminaries.jl,EXPERIMENT=l96_vec,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial preliminaries.sbatch
#
# one array cell (smoke test):
#   sbatch -A esm --array=1-1 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_vec,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial run_array.sbatch
#
# full array:
#   sbatch -A esm --array=1-900 --export=ALL,SCRIPT=run_l96_sbadam.jl,EXPERIMENT=l96_vec,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial run_array.sbatch
#
# leaderboard only:
#   sbatch -A esm --export=ALL,EXPERIMENT=l96_vec,RUN_DATE=<yyyy-mm-dd>,SCORE_KIND=dsm,BUDGET_MODE=serial leaderboard.sbatch
#
# the other arms:
#   SCORE_KIND=dsm       BUDGET_MODE=parallel  bash submit_l96_vec.sh
#   SCORE_KIND=gaussian BUDGET_MODE=serial      bash submit_l96_vec.sh
# ---------------------------------------------------------------------------
